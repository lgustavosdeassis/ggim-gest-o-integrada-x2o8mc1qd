  ...
  return (
    <div className="space-y-6 print-break-inside-avoid">
      <div className="flex items-center gap-3 mb-2">
        ...
        <h3 className="text-xl font-bold text-white">Engajamento e Presenças</h3>
      </div>

      <div className="grid gap-5 grid-cols-2">
        <Card className="border-border/20 shadow-lg bg-card hover:bg-muted/20 transition-all rounded-2xl col-span-2 sm:col-span-1">
          ...
          <CardContent>
            <div className="flex justify-between items-end mt-2">
              <div>
                <div className="text-3xl font-black text-white">{data.pfTotal}</div>
                <p className="text-xs text-muted-foreground font-medium mt-1">Soma Bruta</p>
              </div>
              <div className="text-right border-l pl-5 border-border/30">
        ...
        <Card className="border-border/20 shadow-lg bg-card hover:bg-muted/20 transition-all rounded-2xl col-span-2 sm:col-span-1">
          ...
          <CardContent>
            <div className="flex justify-between items-end mt-2">
              <div>
                <div className="text-3xl font-black text-white">{data.pjTotal}</div>
                <p className="text-xs text-muted-foreground font-medium mt-1">Soma Bruta</p>
              </div>
              <div className="text-right border-l pl-5 border-border/30">
        ...
        <Card className="border-border/20 shadow-lg bg-card hover:bg-muted/20 transition-all rounded-2xl col-span-2">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4 border-b border-border/20 mb-4">
          ...
          <CardContent>
            {data.topPf.count > 0 ? (
              <div className="flex items-center justify-between">
                <div className="text-lg font-bold text-white leading-tight pr-4">
                  {data.topPf.names.join(' / ')}
                </div>
                <div className="shrink-0 text-center px-4 py-2 bg-primary/10 rounded-xl border border-primary/20">
                  <span className="text-2xl font-black text-primary block leading-none">
                    {data.topPf.count}
                  </span>
                  <span className="text-[10px] uppercase tracking-widest font-bold text-primary/80 mt-1 block">
                    Vezes
                  </span>
                </div>
              </div>
      ...
      <Card className="border-border/20 shadow-md bg-background/60 rounded-2xl">
        <CardHeader className="pb-4 border-b border-border/10 mb-4">
        ...
        <CardContent>
          <div className="grid grid-cols-3 gap-2 text-center divide-x divide-border/30">
            <div className="px-2">
              <p className="text-2xl font-black text-white">{data.pfStats.mean.toFixed(1)}</p>
              <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mt-1.5">
                Média
              </p>
            </div>
            <div className="px-2">
              <p className="text-2xl font-black text-white">{data.pfStats.median}</p>
              <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mt-1.5">
                Mediana
              </p>
            </div>
